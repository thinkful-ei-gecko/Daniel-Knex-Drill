-- Begin and commit explicitly set the start and end of a transaction. Here, begin and commit are wrapped around multiple actions. If the first action works, but the second one fails, the commit will not occur, and it will undo the successful command so we are not left with half executed transactions. 
BEGIN;
